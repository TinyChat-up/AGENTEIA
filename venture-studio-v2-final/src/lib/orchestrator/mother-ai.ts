...
export class MotherAI {
    ...
    try {
      ...
      for (const agent of agents) {
        ...
        if (report) {
          ...
          if (!vResult.is_valid) {
            await this.db.from('reports').update({ is_valid: false, validation_errors: vResult.errors }).eq('id', report.id)
            reports_rejected++
          } else {
            await this.db.from('reports').update({ is_valid: true, validation_errors: [] }).eq('id', report.id)
            reports_validated++
          }
        }
        ...
        if (policyResult?.has_blocks) {
          ...
          if (newStatus !== agent.status) {
            await this.db.from('agents').update({ status: newStatus }).eq('id', agent.id)
          }
        }
  ...
  private async evaluateAndScore(agent: Agent): Promise<ScoreBreakdown> {
    ...
    await this.db.from('agents').update({ current_score: score.composite_score }).eq('id', agent.id)
  ...
  private async generateDecision(
    ...
    const { data } = await this.db.from('decisions').insert({
      agent_id:         agent.id,
      outcome,
  ...
  private async applyDecision(agent: Agent, decision: Decision, budget: Budget | null): Promise<void> {
    switch (decision.outcome) {
      case 'kill':
        await this.db.from('agents').update({ status: 'terminated' }).eq('id', agent.id)
        break
      ...
      case 'pivot':
        if (decision.new_hypothesis) {
          await this.db.from('agents').update({ hypothesis: decision.new_hypothesis }).eq('id', agent.id)
        }
        break
    ...
    if (decision.rationale && decision.rationale.length > 30) {
      await this.db.from('learnings').insert({
        agent_id: agent.id,
  ...
  private async promoteAgentStage(agent: Agent, newStage: AgentStage, oldBudget: Budget): Promise<void> {
    ...
    // Update agent stage
    await this.db.from('agents').update({ stage: newStage }).eq('id', agent.id)

    // Create new budget for the new stage
    const { data: newBudget } = await this.db.from('budgets').insert({
      agent_id:                  agent.id,
      stage:                     newStage,
    ...
    if (newBudget) {
      await this.db.from('transactions_ledger').insert({
        agent_id:         agent.id,
  ...
  private async spawnExploiter(explorer: Agent, score: ScoreBreakdown): Promise<void> {
    await this.db.from('agents').update({ status: 'graduating' }).eq('id', explorer.id)

    const { data: exploiter } = await this.db.from('agents').insert({
    ...
    await this.db.from('budgets').insert({
...
## REQUIRED JSON RESPONSE
{
  ...
  private async startCycle(type: OrchestratorRun['cycle_type']): Promise<OrchestratorRun> {
    const { data } = await this.db.from('orchestrator_runs').insert({ cycle_type: type }).select().single()
    return data!
  }

  private async completeCycle(id: string, updates: Partial<OrchestratorRun>): Promise<OrchestratorRun> {
    const { data } = await this.db.from('orchestrator_runs')
  ...
  private async failCycle(id: string, error: string): Promise<void> {
    await this.db.from('orchestrator_runs')
  ...
  private async freezeAgent(id: string, reason: string): Promise<void> {
    await this.db.from('agents').update({ status: 'frozen' }).eq('id', id)
    await this.db.from('decisions').insert({
  ...
  private async recordBudgetChange(agent: Agent, budget: Budget, delta: number, description: string): Promise<void> {
    if (delta > 0) {
      await this.db.from('budgets').update({ allocated_eur: budget.allocated_eur + delta }).eq('id', budget.id)
    }
    await this.db.from('transactions_ledger').insert({
  ...
  private async getActiveAgents(): Promise<Agent[]> {
    const { data } = await this.db.from('agents').select('*').in('status', ['active', 'paused', 'frozen'])
    return (data ?? []) as Agent[]
  }

  private async getBudget(agentId: string): Promise<Budget | null> {
    const { data } = await this.db.from('budgets').select('*').eq('agent_id', agentId).order('created_at', { ascending: false }).limit(1).single()
    return data as Budget | null
  }

  private async getLastReportValid(agentId: string): Promise<boolean | null> {
    const { data } = await this.db.from('reports').select('is_valid').eq('agent_id', agentId).order('created_at', { ascending: false }).limit(1).single()
    return data ? (data as { is_valid: boolean }).is_valid : null
  }

  private async getLatestReport(agentId: string): Promise<ReportV1 | null> {
    const { data } = await this.db.from('reports').select('*').eq('agent_id', agentId).order('created_at', { ascending: false }).limit(1).single()
    return data as ReportV1 | null
  }

  private async getMetrics(agentId: string, days = 30): Promise<MetricsDaily[]> {
    const since = new Date(Date.now() - days * 86400000).toISOString().split('T')[0]
    const { data } = await this.db.from('metrics_daily').select('*').eq('agent_id', agentId).gte('date', since).order('date', { ascending: true })
    return (data ?? []) as MetricsDaily[]
  }

  private async getRuns(agentId: string, limit = 20): Promise<Run[]> {
    const { data } = await this.db.from('runs').select('*').eq('agent_id', agentId).order('created_at', { ascending: false }).limit(limit)
    return (data ?? []) as Run[]
  }

  private async getLearnings(agentId: string): Promise<Learning[]> {
    const { data } = await this.db.from('learnings').select('*').eq('agent_id', agentId)
    return (data ?? []) as Learning[]
  }

  private async getDecisions(agentId: string, limit = 5): Promise<Decision[]> {
    const { data } = await this.db.from('decisions').select('*').eq('agent_id', agentId).order('created_at', { ascending: false }).limit(limit)
    return (data ?? []) as Decision[]
  }

  private async getWeeklySpent(budgetId: string): Promise<number> {
    const since = new Date(Date.now() - 7 * 86400000).toISOString()
    const { data } = await this.db.from('transactions_ledger').select('amount_eur').eq('budget_id', budgetId).lt('amount_eur', 0).gte('created_at', since)
